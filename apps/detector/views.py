...
# login_required, current_use をimportする
...
def detect(image_id):
    ...
    # 物体検知対象の画像パスを取得する
    taget_image_path = Path(
        current_app.config["UPLOAD_FOLDER"], user_image.image_path
    )

    # 物体検知を実行してタグと変換後の画像パスを取得する
    tags, detected_image_file_name = exec_detector(taget_image_path)
...
def exec_detector(target_image_path):
    ...
    for box, label, score, in zip(
        ...
        if score >0.5 and labels[label] not in tags:
            ...
            # 画像にテキストラベルを追記
            cs2 = draw_texts(result_image, line, c1, c2, color, labels, label)
...
# 以下、小川改造分
@dt.route("/run_external", methods=["GET"])
@login_required
def run_external():
    import subprocess

    script_path = Path(current_app.root_path) / "detector" / "Irlen_measurement_v01_r2_syougai.py"
    try:
